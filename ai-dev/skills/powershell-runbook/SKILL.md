...
**これはコードブロックの中だけの話ではない。** チャットの1行に書いた
URL でも同じことが起きる。`https://user.github.io/<リポジトリ名>/` と
書いたら、`%3Cリポジトリ名%3E` を開いて 404 を見た人が「公開できて
いない」と報告してきた。**こちらは調べれば名前が分かる立場にいた。**
埋められる側が埋めるのが筋である。

調べる手段があるなら先に調べて、**開けるものを渡す。** 分からないなら
プレースホルダで濁さず「名前を教えてください」と聞く。この2つだけで、
どちらでもない書き方（埋めさせる）が消える。
